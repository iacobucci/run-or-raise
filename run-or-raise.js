var enabledLinks = ['https://open.spotify.com', 'https://web.whatsapp.com/', 'https://web.telegram.org', 'https://www.google.com/maps'];

//caricamento da storage
...
function handleUpdated(tabId, changeInfo, currentTab) {
	...
	browser.tabs.query({}).then((tabs) => {
		...
		if (matchedTab) {
			...
			//and if the updated website is already present in the opened tabs 
			let movingTab = false;
			for (let tab of tabs) {
			...
			//then switch to the window where the old tab is
			if (movingTab) {
				...
				if (Object.keys(changeInfo).includes("status")) {
					if (changeInfo.status = "loading") {
						...
						console.log(currentTab.windowId, mainWindowId);
						...
						//focus the main window, move currentTab to the main window and then place it in the newPos
